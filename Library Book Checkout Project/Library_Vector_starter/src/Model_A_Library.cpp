...
//the T and U 0are generic params, I can substitute any type for them
...
void reset_books_patrons() {
	...
	string cmd3 = "cp ./dont_alter_these_files/bookfileMT.txt "
			+ BOOKFILE_EMPTY;
	system(cmd3.c_str());

	string cmd4 = "cp ./dont_alter_these_files/patronfileMT.txt "
			+ PATRONFILE_EMPTY;
	system(cmd4.c_str());
}
...
void testfileIO() {
	...
	EXPECT_EQ(COULD_NOT_OPEN_FILE, loadBooks(books, NON_EXISTANT.c_str()), "1",
			THREE_POINTS);

	//return empty for empty file
	EXPECT_EQ(NO_BOOKS_IN_LIBRARY, loadBooks(books, BOOKFILE_EMPTY.c_str()),
			"2", THREE_POINTS);
	...
	//fail on loading bogus file
	EXPECT_EQ(COULD_NOT_OPEN_FILE, loadPatrons(patrons, NON_EXISTANT.c_str()),
			"6", THREE_POINTS);

	//return empty for empty file
	EXPECT_EQ(NO_PATRONS_IN_LIBRARY,
			loadPatrons(patrons, PATRONFILE_EMPTY.c_str()), "7", THREE_POINTS);

	//load real data (make sure there are patrons in file)
	EXPECT_EQ(SUCCESS, loadPatrons(patrons, PATRONFILE.c_str()), "8",
			THREE_POINTS);

	//save it
	EXPECT_EQ(SUCCESS, savePatrons(patrons, TMP_FILE.c_str()), "9",
			THREE_POINTS);
...
void testcheckoutandin() {
	...
	try {
		...
		cout << "Threw exception in howmanybooksdoesPatronHaveCheckedOut"
				<< endl;
	}
